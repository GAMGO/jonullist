...
const BG = require('../../assets/background/home.png'); // 항상 라이트 배경만 사용
...
const RAW = {
  ...
  nexon:    [
    { name: '게임머니 5,000원',   krw: 5000 },
    { name: '게임머니 10,000원', krw: 10000 },
    { name: '게임머니 20,000원', krw: 20000 }
  ],
...
export default function CoinStoreScreen() {
  ...
  return (
    <ImageBackground source={BG} style={{ flex: 1 }} resizeMode="cover">
      <Text style={{ position:'absolute', left:0, right:0, top: insets.top + 8, textAlign:'center', fontFamily: FONT, fontSize: 26, color: '#111' }}>
        {t('STORE') || 'Store'}
      </Text>

      <ScrollView contentContainerStyle={{ paddingTop: insets.top + 70, paddingBottom: insets.bottom + 120, paddingHorizontal: 16 }}>
        {/* 잔액 배지 (길게 누르면 테스트용 +50 코인) */}
        <Pressable
          onLongPress={() => setCoins(c => c + 50)}
          style={{
        ...
        {data.map((cat) => (
          <View key={cat.key} style={{ marginBottom: 18 }}>
            <Text style={{ fontFamily: FONT, color: '#111', fontSize: 18, marginBottom: 8 }}>{cat.title}</Text>
            <View style={{ flexDirection:'row', flexWrap:'wrap', justifyContent:'space-between' }}>
              {cat.items.map((it) => (
                <Card key={it.name} onPress={() => buy(it)}>
                  <RetroArt emoji={cat.emoji} />
                  <Text style={{ fontFamily: FONT, color: '#111', marginTop: 10, marginBottom: 6 }}>{it.name}</Text>
                  <View style={{ flexDirection:'row', alignItems:'center', justifyContent:'space-between' }}>
                    <Text style={{ fontFamily: FONT, color: '#555' }}>{it.krw.toLocaleString()}원</Text>
      ...
      <Modal visible={!!receipt} transparent animationType="fade" onRequestClose={() => setReceipt(null)}>
        <View style={{ flex:1, backgroundColor:'rgba(0,0,0,0.6)', alignItems:'center', justifyContent:'center', padding:16 }}>
          <View style={{ width:'100%', borderRadius:16, backgroundColor: '#fff', borderColor: '#ddd', borderWidth:1, padding:16, gap:10 }}>
            <Text style={{ fontFamily: FONT, color: '#111', fontSize: 18 }}>구매 완료</Text>
            <Text style={{ fontFamily: FONT, color: '#111' }}>상품: {receipt?.item}</Text>
            <Text style={{ fontFamily: FONT, color: '#111' }}>결제 코인: {receipt?.coin}</Text>
            <Text style={{ fontFamily: FONT, color: '#22c55e' }}>바우처 코드: {receipt?.code}</Text>
            <Pressable onPress={() => setReceipt(null)} style={{ backgroundColor: '#111827', padding:12, borderRadius:10, alignItems:'center' }}>
